<template>
  <div class="main">
    ...
    <!--商品排序-->
    <good-sort v-if="showSort" @childsort="sortdata"></good-sort>
  </div>
</template>
...
import GoodSort from './goodsSort'
export default {
  name: 'takeawayDudit',
  components:{
    viewGoods,GoodSort
  ...
  methods:{
    ...
    delgoods(){
      ...
      let url='/api/web/takeout-product/delete',
        params={
          products:productsID,
          status:'4'//1[待审核] 2[审核拒绝] 3[回收站] 4[上架] 5[下架]
        }
      vm.$axios({
        method:'post',
        data:params,
