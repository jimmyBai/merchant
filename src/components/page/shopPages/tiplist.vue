      ...
      vm.$axios.get(url,{params}).then((res)=>{
        if(res.data.error_code=='0'){
          if(res.data.data){
            ...
            vm.amount_setting=JSON.parse(res.data.data.amount_setting)
          }
        }else{
...
<style scoped>
.tipcontent{padding: 10px; background: #38213e; }
.formbox{ margin: 0 30px 100px 30px; }
...
.line .tipprompt{ display: block;font-size: 12px;}
.firstIp{ margin: 0 5px 0 0!important; }
.formbox .switch{ margin: 15px 0; }
.switch .content{margin-top: 10px;}
.formbox .blessing{margin: 15px 0; }
.blessing .content{margin: 15px 0; }
.blessing .content textarea { background: none; height: 80px;  outline:none; border: none; width: 100%; resize: none; color: #fff; text-indent: 10px; -webkit-appearance: none; }
.formbox .charge{ margin: 15px 0;}
.formbox .content{ margin: 15px 0;}
.greetbox{width: 400px;border:1px solid #706375; border-radius: 3px; padding: 10px; }
.savebtn{margin: 40px 0 15px 0;}
.savebtn input{cursor: pointer;  background: #ac5397; height: 40px;  line-height: 40px; color: #fff;  display: inline-block; width: 400px;  text-align: center; border: 0; font-size: 12px; -webkit-appearance: none; }
